...
export default function Product() {
  ...
  const handleFilter = (value , sectionId)=>{
    ...
    console.log(filtervalue , searchParams , value , sectionId);
  ...
  const handleSort = (sectionId,value)=>{
    const searchParams = new URLSearchParams(location.search);
  ...
  // colors, sizes, minPrice, maxPrice, category, stock, minDiscount, sort, pageNumber, pageSize

  useEffect(()=>{
    const [minPrice , maxPrice]=priceValue===null?[0,Infinity]:priceValue.split('-').map(Number);
    const data=  {
  ...
  }, [colorValue, sizeValue , stockValue, discountValue , priceValue , param.levelThree , pageNumber , sortValue]);
  return (
              ...
              >
                <Dialog.Panel className="relative ml-auto flex h-full w-full max-w-xs flex-col overflow-y-auto bg-white py-4 pb-12 shadow-xl">
                  <div className="flex items-center justify-between px-4">
                    <h2 className="font-semibold text-gray-600 text-lg">Filters</h2>
                    <button
                      type="button"
                        ...
                        {({ open }) => (
                          <>
                            <h3 className="-mx-2 -my-3 flow-root">
                              <Disclosure.Button className="flex w-full items-center justify-between bg-white text-bold px-2 py-3 text-gray-900 hover:text-gray-500">
                                <span className="text-lg   text-gray-900">{section.name}</span>
                        ...
                        <Menu.Item key={option.name}>
                          {({ active }) => (
                            <a
                              ...
                              onClick={()=>handleSort('sort' , option.href)}
                          ...
                          <div className="space-y-4">
                            {section.options.map((option, optionIdx) => (
                              <div key={option.value} className="flex items-center">
                                ...
                                <label
                                  htmlFor={`filter-${section.id}-${optionIdx}`}
                                  className="ml-3 text-sm text-bold text-gray-900"
                                >
                                  {option.label}
                                </label>
                              </div>
                  ...
                  <Disclosure as="div" key={section.id} className="border-b border-gray-200 py-6 w-full">
                    {({ open }) => (
                      <FormControl className="w-full">
                        <h3 className="-my-3 flow-root ">
                          <Disclosure.Button  className="flex  items-center justify-between bg-white py-3 text-sm text-gray-900 hover:text-gray-500 w-full">
                        ...
                        <Disclosure.Panel className="pt-6">
                          <div className="space-y-4">
                                ...
                                >
                                {section.options.map((option, optionIdx) => (
                                  <FormControlLabel  value={option.value} control={<Radio />}  label={option.label} onChange={(e)=>{handleRadioFilter(e , section.id)}} />
                                ))}
